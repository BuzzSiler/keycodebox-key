...
void CSystemController::OnCardSwipe(QString sCode1, QString sCode2)
{
    ...
    if(_systemState == ELockerScreen ) {
      ...
      /*
	the below instructions are still required fir ELMS variant,
	inputting passwords using a magnetic card in the admin for instance:
      */
      QString xdotool = "xdotool key ";
      QString tempCmd = xdotool + sCode1[0];
      std::system(tempCmd.toStdString().c_str());
      usleep(5);
      tempCmd = xdotool + sCode1[1];
      std::system(tempCmd.toStdString().c_str());
      usleep(5);
      tempCmd = xdotool + sCode1[2];
      std::system(tempCmd.toStdString().c_str());
      usleep(5);
      tempCmd = xdotool + sCode1[3];
      std::system(tempCmd.toStdString().c_str());
      usleep(5);
      tempCmd = xdotool + sCode1[4];
      std::system(tempCmd.toStdString().c_str());
      usleep(5);
    }
...
//    _securityController.moveToThread(_pSecurityThread);
    ...
    connect(&_securityController, SIGNAL(__OnSecurityCheckSuccess(int,bool)), this, SLOT(OnSecurityCheckSuccess(int,bool)));
...
void CSystemController::OnCodeEntered(QString sCode)
{
    ...
    if( sCode.length() != 5 )
      emit __OnCodeMessage("Too short! Must be 5 digits!", false);
    else
      {
...
//    _systemState = EAdminPasswordFailed;
  emit __OnCodeMessage("Incorrect Password",false);
    emit __OnClearEntry();
...
void CSystemController::OnOpenLockRequest(int nLockNum)
{
    ...
    if( nRC == 1 )
    {
        // Open the lock
        qDebug() << "Lock Open";
        emit __OnCodeMessage("Lock Open",false);
        //_LockController.openLockWithPulse(nLockNum, 2, 50, 50);
...
void CSystemController::OnSecurityCheckSuccess(int lockNum, bool lockboxState)
{
      ...
      if( _systemState != ETimeoutScreen )
	{
	  ...
	  emit __OnCodeMessage("Lock Open", lockboxState);
	}
        reportActivity();
    }
}
...
//    _systemState = EPasswordFailed;
  emit __OnCodeMessage("Incorrect Code", false);
    emit __OnClearEntry();

    QTimer::singleShot(10000, this, SLOT(resetCodeMessage()));
...
void CSystemController::resetCodeMessage()
{
    if(_systemState == EUserCodeOne) {
      emit __OnCodeMessage("<Please Enter Code #1>", false);
    } else if(_systemState == EUserCodeTwo) {
      emit __OnCodeMessage("<Please Enter Second Code>", false);
    }
}
...
{
    _systemState = ETimeoutScreen;
    emit __OnCodeMessage("Password Timeout",false);
    emit __OnClearEntry();
}
...
void CSystemController::OnLockerScreen()
{
  qDebug() << "CSystemController::OnLockerScreen()";

  _systemState = ELockerScreen;
...
//        connect(this, SIGNAL(__OnCodeMessage(QString,bool)), _pfUsercode, SLOT(OnNewCodeMessage(QString,bool)));
    }
...
void CSystemController::looprun()
{
    ...
    if( _systemState == ELockerScreen ) {
	  ...
	  if( _systemStateDisplay != EUserCodeTwo ) {
        ...
        if( _systemStateDisplay != EAdminPassword ) {
            ...
            stopTimeoutTimer();
            emit __OnCodeMessage("<Enter Admin Password>",false);
	...
	emit __OnClearEntry();
	//emit __OnCodeMessage(str,false);
	emit __OnNewMessage("");
	stopTimeoutTimer();

	// THIS SETS HOW LONG THE THANK YOU MESSAGE HOLDS
	sleep(8);
