...
/**
 * @brief CTblCodes::getFullLockersElms
 * @param capacity - number of total storage lockers
 */
int *CTblCodes::getFullLockersElms(int capacity)
{
  qDebug() << "CTblCodes::getFullLockersElms()";

  int c;
  int *fullLockers = (int *)malloc((size_t)(sizeof(int) * capacity));
  ...
  if( _pDB && _pDB->isOpen() )
	...
	qDebug() << "qry.prepare fails!" << qry.lastError();
      ...
      int lockboxNum; // temporarily hold the lockbox number of last one
      ...
      if(qry.exec())
	{
	  ...
	  while(qry.next())
	    {
	      lockboxNum = qry.value(fldLockNo).toInt();
	     
	      qDebug() << "CTblCodes::getFullL0ckerElms(), lockboxNum: " << QString::number(lockboxNum);
	      
	      //set lockbox number state to appropriate value
	      fullLockers[lockboxNum-1] = 1;
	    }
	}
      else
	{
...
/**
 * @brief CTblCodes::checkCodeOneElms
 * @param code - encrypted
 */
int CTblCodes::checkCodeOneElms(std::string code, int &nLockNum )
{
    qDebug() << "CTblCodes::checkCodeOneElms()";

    bool bSecondCodeRequired = false;
...
//        }
	    ...
	    int fldLockboxState = qry.record().indexOf("lockbox_state");
            qDebug() << "fldLockNo:" << fldLockNo << "  fldCode1No:" << fldCode1No << "  fldCode2No:" << fldCode2No;
            qDebug() << "Count: " << qry.size() << " Fingerprint1: " << fldFingerprint1No << " LockboxState: " << fldLockboxState;
            while(qry.next())
            {
                // Check how many we have.
                // If only one then see if it requires a second code.
		    ...
		    if( qry.value(fldLockboxState).toInt() == 0)
		      {
...
{
  qDebug() << "CTblCodes::selectCodeSet(),  nLockNum: " << QString::number(nLockNum);;
    // hold on to the code
    CLockState  *pLock;
    *pLockSet = 0;
...
{
  qDebug() << "CTblCodes::selectCodeSet(ids, pLockSet), ids: " << QString::number(ids);
    // hold on to the code
    CLockState  *pLock;
    *pLockSet = 0;
                           ...
                           QDateTime starttime, QDateTime endtime)
{
    ...
    QString sql = "DELETE FROM " + QString(TABLENAME.c_str()) +
            QString(" WHERE locknum=:locknum");

    qDebug() << "CTblCodes::deleteCode sql:" << sql;
    qDebug() << "CTblCodes::deleteCode locknum:" << locknum;

    query.prepare(sql);
    query.bindValue(":locknum", locknum);
    if( query.exec()) {
        qDebug() << "CTblCodes::deleteCode(" << code1 << ") succeeded";
        return true;
    } else {
        qDebug() << "CTblCodes::deleteCode(" << code1 << ") failed";
        return false;
    }
}
...
bool CTblCodes::updateLockboxState(int fids, bool lockstate)
{
  ...
  qDebug() << "CTblAdmin::updateLockbox(), query: " << sql;
    ...
    if(qry.exec()) {
        ...
        qDebug() << "CTblCodes::updateLockbox() failed";
        return false;
    }
}
