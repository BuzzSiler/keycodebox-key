...
CMagTekCardReader::CMagTekCardReader(QObject *parent) : QObject(parent),
...
bool CMagTekCardReader::openDeviceHandle()
{
  ...
  int nCountDevs = _usbController->CountDevicePorts(deviceType, filterString);
  if( nCountDevs > 1 ) {
    // Can't continue
  ...
  std::string sDevNum = _usbController->getDevicePortString(deviceType, filterString, devicePrefix);
		      ...
		      if( currentCodeLength > 4 )
			{
			  ...
			  if( consecutiveZeros < 4)
			      ...
			      for(idx=0; idx<5; idx++)
				{
			      ...
			      finalStrCode = QString::number(finalCode[0]) + QString::number(finalCode[1]) + QString::number(finalCode[2]) +
				QString::number(finalCode[3]) + QString::number(finalCode[4]);
			      
			      emit __onCardSwipe(QString(finalStrCode), "");
